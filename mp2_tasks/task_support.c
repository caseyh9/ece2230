 ...
 *
 *    type_of_list: a character string that must be either "List" or "Queue"
 *
...
{
	// Get the size of each list using the list_size() function
    int num_in_sorted_list = list_size(sorted);
    int num_in_unsorted_list = list_size(unsorted);
	// Get the order of the sorted list with list_order()
