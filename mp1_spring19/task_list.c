...
{
	enum state current_state = -1;	// FIXME is this allowed?
	int i = 0;
...
struct task_list_t* task_list_remove_finished(struct task_list_t *list_ptr)
{
	...
	for (i = 0; i < list_ptr->list_size; i++)
	{
		if (list_ptr->task_ptr[i]->state == FINISHED)
		{
			rm->task_ptr[task_count] = task_list_remove(list_ptr, i);
			task_count++;
			i--;
		}
	}
